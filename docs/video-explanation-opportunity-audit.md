...
| Math1030 | `eigenvalues/diagonalization-and-similarity` | Done: put a source eigenbasis into `S`, match eigenvalues in `D`, show `CS=SD`, and convert it into `S^{-1}CS=D`. | Implemented as `math1030-diagonalization-similarity-eigenbasis-story`; characteristic-polynomial and multiplicity tests remain in the next 8.3 slice. |
