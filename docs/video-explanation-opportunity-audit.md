...
| Math1030 | `math1030.matrices.existence-of-row-echelon-forms` | `math1030-existence-of-row-echelon-forms-story` | Implemented | The optional existence proof is a compact proof-as-algorithm sequence: place a pivot, reduce the smaller block by induction, clean REF to RREF, and preserve pivot columns. |
...
| 8 | Math1030 | `determinants/row-operations-products-and-invertibility` | Done: animate row swaps, scaling, row replacement, triangular read-off, zero-row singularity, and the elementary-matrix bridge to product / invertibility theorems. | Implemented as `math1030-determinants-row-operation-effects-story`; no widget added because the source-backed learning move is operation-effect bookkeeping. |
| 9 | Math1030 | `eigenvalues/diagonalization-and-similarity` | Done: show source eigendirections becoming the columns of `S`, package the equations as `CS=SD`, convert to `S^{-1}CS=D`, and read diagonal action in eigenvector coordinates. | Implemented as `math1030-diagonalization-similarity-eigenbasis-story`; no widget added because the source-backed value is the fixed eigenbasis-to-similarity bridge. |
...
| Math1030 | `matrices/existence-of-row-echelon-forms` | Done: REF existence by row-count induction, REF-to-RREF cleanup by rank induction, and pivot-column preservation. | Implemented as `math1030-existence-of-row-echelon-forms-story`; no widget added because this unit explains a fixed proof mechanism, not an open-ended row-reduction trace. |
...
| Math1030 | `vector-spaces/basis-extension-and-change-of-basis` | Done: two ordered bases on the same proper subspace, columns of `S` as coordinate recipes, `U=VS`, and `[x]_V=S[x]_U` with inverse direction warning. | Implemented as `math1030-basis-extension-change-of-basis-story`; no widget added because the source-backed value is a fixed theorem/example bridge, not open-ended coordinate exploration. |
| Math1030 | `determinants/determinants-and-cofactor-expansion` | Done: minor deletion, cofactor checkerboard signs, the source-backed `3x3` expansion tree, zero pruning, one-nonzero collapse, and triangular read-off. | Implemented as `math1030-determinants-cofactor-expansion-story`; no widget added because the source-backed value is a curated recursive computation strategy. |
| Math1030 | `determinants/row-operations-products-and-invertibility` | Done: three row-operation determinant effects, source-backed row-replacement triangular example, zero-row determinant `0`, `det(EA)=det(E)det(A)`, product determinant, and invertibility test. | Implemented as `math1030-determinants-row-operation-effects-story`; no widget added because the first pass should teach the determinant ledger, not expose an open-ended elimination calculator. |
| Math1030 | `determinants/transpose-column-operations-and-cramers-rule` | Done: read `Ax=b` by columns, replace one column by `b`, split by column linearity, cancel repeated-column determinants, and apply the source Cramer example. | Implemented as `math1030-determinants-column-replacement-cramers-rule-story`; no widget added because the source-backed value is the fixed numerator-isolation proof, not an open-ended determinant calculator. |
| Math1030 | `eigenvalues/eigenvalues-eigenvectors-and-eigenspaces` | Done: contrast ordinary vectors with preserved eigen-directions, then convert `Av=λv` into `(A-λI)v=0` and identify `E_A(λ)=N(A-λI)`. | Implemented as `math1030-eigenvalue-direction-eigenspace-story`; no widget added because the source-backed value is the fixed direction-to-null-space bridge. |
| Math1030 | `eigenvalues/diagonalization-and-similarity` | Done: put a source eigenbasis into `S`, match eigenvalues in `D`, show `CS=SD`, and convert it into `S^{-1}CS=D`. | Implemented as `math1030-diagonalization-similarity-eigenbasis-story`; no widget added because the source-backed value is the fixed eigenbasis-to-similarity bridge. |
| Math1030 | `eigenvalues/characteristic-polynomials-and-diagonalization-tests` | Done: package `det(A-lambda I)=0` as `p_A(x)=det(A-xI)`, factor the source 2 by 2 example, contrast algebraic and geometric multiplicity, and finish with the dimension-sum / distinct-eigenvalue tests. | Implemented as `math1030-characteristic-polynomial-diagonalization-test-story`; no widget added because the source-backed value is a curated decision chain, not an open-ended characteristic-polynomial calculator. |
| Math1030 | `inner-products/inner-products-norms-and-angles` | Done: turn the standard inner product into the norm, unit-vector normalization, and the nearby angle / orthogonality bridge. | Implemented as `math1030-inner-product-norm-angle-story`; no widget added because the first pass is a fixed algebra-to-geometry bridge, and projection subtraction is reserved for Gram-Schmidt. |
| Math1030 | `inner-products/orthogonal-sets-and-orthonormal-bases` | Done: orthogonal vectors, pairwise orthogonal sets, automatic independence, coordinate reading, and normalization to an orthonormal basis. | Implemented as `math1030-orthogonal-sets-orthonormal-bases-story`; no widget added because the source-backed learning move is coefficient isolation by inner products. Projection subtraction remains in Gram-Schmidt. |
