...
呢個流程唔只係程式風格，而係資料結構程式最重要的安全習慣。當你之後見到
stack node、queue node、或者 hashtable cell，都應該即刻問：呢個 object 邊
度來？邊度去？
...
student 例子值得再慢讀一次，因為它同時牽涉 pointer、struct、file input
同 ownership。
...
當你可以清楚答到呢四條，代表你已經開始將 pointer、field access 同
resource ownership 放在同一個框架裡面理解，而唔係逐句死記。
